"""
scripts/visualize_model_b.py — Visual evaluation of Swin UNETR
...
# ─────────────────────────────────────────────────────────────────────────────
# Grad-CAM — hooked on decoder1 (last full-res decoder block, ~96³)
# ─────────────────────────────────────────────────────────────────────────────

class GradCAM:
    """Grad-CAM on SwinUNETR decoder1 — full 96³ spatial resolution."""

    def __init__(self, model):
        self.model = model
        self._feats = self._grads = None
        # model.swin.decoder1 is the final UnetrUpBlock before the output head
        # It outputs [B, feature_size, 96, 96, 96] at full resolution
        hook_layer = model.swin.decoder1
        self._h1 = hook_layer.register_forward_hook(
            lambda m, i, o: setattr(self, "_feats", o.detach()))
        self._h2 = hook_layer.register_full_backward_hook(
...
# ─────────────────────────────────────────────────────────────────────────────
# Data helpers (identical to model_a version)
# ─────────────────────────────────────────────────────────────────────────────
...
def load_and_preprocess(case_id, brats_root, patch_size=(96, 96, 96)):
    ...
    # Find tumour centre (WT) for patch extraction
    wt = label[0]
    if wt.sum() > 0:
...
def pick_three_slices(label_t):
    """Return z-indices at 25/50/75% of WT tumour z-range. label_t: [3,H,W,D]."""
    wt = label_t[0].numpy()           # [H, W, D]
    per_z = wt.sum(axis=(0, 1))       # [D]
...
def make_figure(case_id, img_t, label_t, pred_bin, cam, t1ce_raw):
    slices = pick_three_slices(label_t)
...
def main():
    ...
    parser.add_argument("--checkpoint",  default="checkpoints/model_b/best.pth")
    ...
    model = SwinUNETRWrapper(in_channels=4, out_channels=3).to(device)
